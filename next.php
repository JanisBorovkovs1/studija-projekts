                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <?php if ($row['owner_id'] == $_SESSION['id_users']): ?>
                            <div class="mt-3 d-flex gap-2">
                                <a href="edit_listing.php?id=<?= $row['id_listings'] ?>" class="btn btn-sm btn-outline-secondary">
                                    ✏️ Labot
                                </a>

                                <form action="delete_listing.php" method="POST" onsubmit="return confirm('Vai tiešām vēlies izdzēst šo sludinājumu?');" style="display:inline;">
                                    <input type="hidden" name="id" value="<?= $row['id_listings'] ?>">
                                    <button type="submit" class="btn btn-sm btn-danger d-flex align-items-center gap-2 shadow-sm">
